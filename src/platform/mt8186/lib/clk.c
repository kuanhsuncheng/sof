...
static void set_mux_sel(enum mux_id_t mux_id, uint32_t value)
{
	switch (mux_id) {
...
static int clock_platform_set_dsp_freq(int clock, int freq_idx)
{
	switch (freq_idx) {
	case ADSP_CLK_26M:
		set_mux_sel(MUX_CLK_ADSP_SEL, MTK_CLK_ADSP_26M);
		break;
	case ADSP_CLK_PLL_800M_D_8:
		set_mux_sel(MUX_CLK_ADSP_SEL, MTK_CLK_ADSP_DSPPLL_8);
		break;
	case ADSP_CLK_PLL_800M_D_4:
		set_mux_sel(MUX_CLK_ADSP_SEL, MTK_CLK_ADSP_DSPPLL_4);
		break;
	case ADSP_CLK_PLL_800M_D_2:
		set_mux_sel(MUX_CLK_ADSP_SEL, MTK_CLK_ADSP_DSPPLL_2);
		break;
	case ADSP_CLK_PLL_800M:
		set_mux_sel(MUX_CLK_ADSP_SEL, MTK_CLK_ADSP_DSPPLL);
		break;
	default:
		set_mux_sel(MUX_CLK_ADSP_SEL, MTK_CLK_ADSP_26M);
		break;
...
void platform_clock_init(struct sof *sof)
{
	...
	/* DSP bus clock */
	set_mux_sel(MUX_CLK_ADSP_BUS_SRC_SEL, MTK_ADSP_CLK_BUS_SRC_EMI);
}
