...
void platform_timer_start(struct timer *timer)
{
	...
	//io_reg_write(CNTCR, TIMER_CLK_SRC_CLK_26M << TIMER_CLK_SRC_SHIFT);

	/*enable timer*/
	io_reg_update_bits(CNTCR, CNT_EN_BIT, CNT_EN_BIT);
	io_reg_update_bits(OSTIMER_CON, CNT_EN_BIT, CNT_EN_BIT);
}
...
static int platform_timer_register(struct timer *timer, void (*handler)(void *arg), void *arg)
{
	...
	timer->hitime = 0;
	timer->hitimeout = 0;
	err = interrupt_register(timer->irq, platform_timer_handler, timer);
	if (err < 0)
		return err;

	/* enable timer interrupt */
	interrupt_enable(timer->irq, timer);
